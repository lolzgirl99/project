...
--modify our map bm
 ...
 --From here the situation is under the assumption that the variety is not contained in any hyperplane.
    ...
    --if (o.Verbose) then print toString(barJD);
...
nonZeroMinor(Matrix,ZZ,ZZ):=o->(M,ra,minorCount)->(
    ...
    bestDegree := chooseSubmatrixLargestDegree(ra, M);
    rowList = bestDegree#0;
    colList = bestDegree#1;
    subM = (M^rowList)_colList;
    --curRank = rank(subM);
    if (not (det subM == 0)) then (--change this a bit...
         if (o.Verbose==true) then print "nonZeroMinor: Found a nonzero minor non-randomly";
         flag = true;
         curRank = ra;
    );
    if (o.Verbose==true) then print "nonZeroMinor: looking for a random nonzero minor";
...
--    rank matrix(frac(theRing), {L1, L2}) == 1
 ...
 isSameMap(List, List, Ring) := (L1, L2, R1) -> (
    print toString(matrix(frac(R1), {L1, L2}));
